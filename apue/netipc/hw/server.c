...
int main(void)
{
	...
	while(1)
	{
		...
		if(0 == pid)
		{
			...
			cnt = read(accepted_socket, &rcvbuf, MAXMSG);
			if(-1 == cnt)
			{
				perror("read()");
				exit(1);
			...
			while(1)
			{
				...
				{
					close(fd);	
					write(accepted_socket, "!end", 4);
					break;
				}
				write(accepted_socket, rcvbuf.msg, cnt);
			}
